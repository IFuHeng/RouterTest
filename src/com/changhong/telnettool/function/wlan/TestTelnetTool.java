 ...
 *
 */
public class TestTelnetTool extends JFrame implements ActionListener, WindowStateListener, Runnable, ItemListener {
    ...
    /************* value *********************/
    private java.util.List<CommandBeen> mDataCommand = new ArrayList<>();
    private final java.util.List<Button> mListButtonDel = new ArrayList<>();
    private final java.util.List<Checkbox> mListCheckboxEnable = new ArrayList<>();
    private final java.util.List<Checkbox> mListCheckboxUnlimited = new ArrayList<>();
    private ExecutorService mThreadPool;

    private TextField mViewOutPutField;
    private Button mBtnExport;

    public TestTelnetTool() {
        super("WIFI自动化工具");
...
//        setMenuBar(menuBar);
    ...
    private void initCommandView() {
        mViewListCommand.setBackground(Color.gray);
        mViewListCommand.setSize(800, 500);
        mViewListCommand.setMaximumSize(new Dimension(800, 500));
        createCmdTitle(mViewListCommand);
        mDataCommand = DataManager.load();
        for (int i = 0; i < mDataCommand.size(); ++i)
            createNewCmdRow(mViewListCommand, i, mDataCommand.get(i));

        Panel panel = new Panel(new BorderLayout());
        panel.setMinimumSize(new Dimension(800, 500));
        ScrollPane scrollPane = new ScrollPane();
        scrollPane.add(mViewListCommand);
        scrollPane.setSize(800, 300);

        Label title = new Label("Command list:");
        title.setBackground(Color.LIGHT_GRAY);
        title.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 16));

        panel.add(title, "North");
        panel.add(scrollPane, "Center");
        panel.add(createAddCmdBar(), "South");
        this.add(panel, "Center");
    }

    private Component createAddCmdBar() {
        GridBagLayout layout = new GridBagLayout();
        Panel panel = new Panel(layout);

        mGBC.gridwidth = 1;
        panel.add(new Label("Cmd:"));

        mViewCmdAdd = new TextField();
        mViewCmdAdd.setColumns(40);
        mGBC.fill = GridBagConstraints.HORIZONTAL;
        layout.setConstraints(mViewCmdAdd, mGBC);
        panel.add(mViewCmdAdd);

        mGBC.fill = GridBagConstraints.NONE;

        mGBC.gridwidth = GridBagConstraints.REMAINDER;
        mBtnAdd = new Button(ACTION_ADD);
        mBtnAdd.setActionCommand(ACTION_ADD);
        mBtnAdd.addActionListener(this);
        layout.setConstraints(mBtnAdd, mGBC);
        panel.add(mBtnAdd);
        ...
        mGBC.fill = GridBagConstraints.HORIZONTAL;
        layout.setConstraints(mViewOutPutField, mGBC);
        ...
        layout.setConstraints(mBtnExport, mGBC);
        panel.add(mBtnExport);
        Insets insets = panel.getInsets();
        Tool.log(panel.getInsets());
    ...
    public void run() {
        ...
        try {
            ...
            mViewResponse.append(telnetManager.login("root", "admin2020@ch"));
        } catch (Exception e) {
            e.printStackTrace();
